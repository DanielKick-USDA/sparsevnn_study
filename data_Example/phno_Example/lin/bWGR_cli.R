...
if (length(args)<2) {
  stop("Expected unamed arguments `hash`:string `inp_dir`:string `out_dir`:string `max_inp`:int\n If `max_inp` <= 0 filtering will be skipped ", call.=FALSE)
...
#print('Running with `hash`, `out_dir` `max_inp`')
...
max_inp <- get_arg(args, key='--max_dir', '0')

max_inp <- as.integer(max_inp)
