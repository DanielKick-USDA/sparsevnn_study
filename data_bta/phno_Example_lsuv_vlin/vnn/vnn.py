...
## Custom verisons of functions 
# need to include `evaluate` here because I updated `train_one_model` to use `sparsevnn_lsuv`
...
def sparsevnn_lsuv(
    ...
    # run initialization with orthonormal
    for L in range(len(M_list)):
        for node_out_name in M_list[L].col_info.keys():
            # for each output in col_info, find all the inputs in row_info
            node_out_start, node_out_stop, node_out_size = [
                int(M_list[L].col_info[node_out_name][e]) 
                for e in ['start', 'stop', 'size']
                ]

            # unlike the presumably slower approach of identifying input nodes based on indexing 
            # there appear to be dead connections in the edge_dict that we'll have to ignore. 
            if node_out_name not in edge_dict.keys():
                pass
            else:
    ...
    # Now update each layer sequentially using LSUV
    for L in tqdm(range(len(M_list))):
        # make sure this is in training mode
        model = model.eval()
        ...
        for node_out_name in M_list[L].col_info.keys():
            ...
            # Create mask for the indices
            node_mask_indices = (
                # only index based on output to scale all of the relevant weights together
                ((model.layer_list[L].indices[0] >= node_out_start) & (model.layer_list[L].indices[0] <  node_out_stop)) 
                )

            # if node_out_size > 2???
            for ii in range(trial_max):
                # calculate the current output for he current batch
                with torch.no_grad():
                    x = next(iter(inp_dl))[1]
                    # can we just add 1 to get the weights to do things?
                    x += 1.0
                    model = model.to(x.device.type) # make sure the model and data are on the same device
                    for Li in [e for e in range((L+1))]:
                        if Li == 0:
                            out = model.layer_list[Li](x)
                        else:
                            out = model.layer_list[Li](out)

                node_mb_xbar = out[:, node_out_start:node_out_stop].detach().mean()
                node_mb_sd   = out[:, node_out_start:node_out_stop].detach().std()

                if verbose:
                    if ii == 0:
                        print(f"{node_out_start}, {node_out_stop}, {node_out_name}")
                        print(f"xbar\t\t\tsd")
                        print(f"{node_mb_xbar}\t{node_mb_sd}")
                        print("_________________________________")
                    else:
                        print(f"{node_mb_xbar}\t{node_mb_sd}")

                # if sd = 0, take no action. 
                if (node_mb_sd == 0.0):
                    break
                elif (abs(float(node_mb_sd) - 1) < 0.01):
                    break
                else:
                    with torch.no_grad():
                        # added 1e-8 a la https://github.com/ducha-aiki/LSUV-pytorch/blob/master/LSUV.py#L130
                        model.layer_list[L].weights[node_mask_indices] /= (node_mb_sd + 1e-8) 

        return model


# activation function is in `train_one_model` and `vnn_from_state_dict`
# `vnn_from_state_dict` should not be run now that data is being pickled.
def train_one_model(
    params, # = params,
    ...
    def _propose_model(
        ...
        layer_list = info_list_to_layer_list(M_list = M_list, nonlinearity = None)
        model      = SparseVNN(layer_list = layer_list)


        # apply lsuv
        print('\nApplying LSUV\n')
        model = sparsevnn_lsuv(
            model = model,
            M_list = M_list,
            edge_dict = myvnn.edge_dict,
            inp_dl = training_dataloader,
            tol_var = 0.01,
            trial_max = 20
            )
        print('\nFinished LSUV\n')
    ...
    pr_uniq_threshold  = 0.95 # at .99 failed with F.tanh and 100 iterations. Leaky relu also failed. 
    check_model_trials = 1 # set to only run once since LSUV is now being applied. 
    ...
    if True:
        ...
        VNN.configure_optimizers(weight_decay=1.0)
...
## right now activation function is embedded in 
