...
# 250228 now that the maize meeting is off, this is going to be poorly named and hastely done - sorry future daniel. 





mk_hyps_block <- function(M = M, xx_exp = "data_zma", xx_trait = "phno_all_Pollen_GDD__FTaxaREnvRGxE"){
  
  yy <- list(
    c("dnn", paste0(xx_exp, ''),           xx_trait, "DNN Tanh"),
    c("vnn", paste0(xx_exp, ''),           xx_trait, "VNN Tanh"),
    c("vnn", paste0(xx_exp, '_lsuv_tanh'), xx_trait, "VNN Tanh LSUV"),
    c("vnn", paste0(xx_exp, '_lsuv_relu'), xx_trait, "VNN Relu LSUV"), 
    c("vnn", paste0(xx_exp, '_lsuv_vlin'), xx_trait, "VLN LSUV") 
  )
  plt_list <- map(seq_along(yy), function(j){
    plt <- plt_hyps(
      df = filter(
        M,
        model == yy[[j]][1],
...
tmp_fn <- function(M = M, 
                   xx_exp = "data_zma", 
                   xx_trait = "phno_all_Pollen_GDD__FTaxaREnvRGxE"){
  p <- mk_hyps_block(M = M, xx_exp = xx_exp, xx_trait = xx_trait)
  design <- "ABCDE"
  p <- (p[[1]]+p[[2]]+p[[3]]+p[[4]]+p[[5]])+plot_layout(design = design)
  return(p)
}
...
walk(
  seq(1, nrow(xx)), function(i){
    p <- all_plts[[i]]
    ggsave(paste0('paper_0hyps_1grid_', xx[i, 'exp'], '-', xx[i, 'phn'], '.svg'), 
           p,  width = 30, height = 5, units = 'in')
})
...
walk(
  seq(1, nrow(xx)), function(i){
    p <- all_plts[[i]]
    ggsave(paste0('paper_0hyps_1grid_', xx[i, 'exp'], '-', xx[i, 'phn'], '.png'), 
           p,  width = 30, height = 5, units = 'in')
})

# names(all_plts) <- xx$phn
# 
# p <- all_plts
# 
# design <- "
...
# Poorly abstracted because of relabeling


p <- plt_net_metrics(md = md, phase_i = 'lightning', phno_i = 'ADHReplicates_hzg')
p <- p+scale_y_continuous(
        trans = "log10",
        breaks = c(0.5, 1, 10, 100, 1000))+
    labs(x = '', y = '')+
  facet_grid(phno~init, labeller = labeller(
  phno = c(ADHReplicates_hzg = "ADH Slope"),
  init = c(
    dnn.r.tanh = 'DNN Tanh',
    vnn.r.tanh = 'VNN Tanh',
    vnn.l.tanh = 'VNN Tanh LSUV',
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_0hyps_2trace_data_dme-ADHReplicates_hzg.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_0hyps_2trace_data_dme-ADHReplicates_hzg.png', p, width = 30, height = 5, units = 'in')


p <- plt_net_metrics(md = md, phase_i = 'lightning', phno_i = 'phno_ProteinDry')
p <- p+scale_y_continuous(
        trans = "log10",
        breaks = c(0.5, 1, 10, 100, 1000))+
    labs(x = '', y = '')+
  facet_grid(phno~init, labeller = labeller(
  phno = c(phno_ProteinDry = "Protein Dry"),
  init = c(
    dnn.r.tanh = 'DNN Tanh',
    vnn.r.tanh = 'VNN Tanh',
    vnn.l.tanh = 'VNN Tanh LSUV',
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_0hyps_2trace_data_gmx-phno_ProteinDry.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_0hyps_2trace_data_gmx-phno_ProteinDry.png', p, width = 30, height = 5, units = 'in')


p <- plt_net_metrics(md = md, phase_i = 'lightning', phno_i = 'phno_StachyoseDry')
p <- p+scale_y_continuous(
        trans = "log10",
        breaks = c(0.5, 1, 10, 100, 1000))+
    labs(x = '', y = '')+
  facet_grid(phno~init, labeller = labeller(
  phno = c(phno_StachyoseDry = "Stachyose Dry"),
  init = c(
    dnn.r.tanh = 'DNN Tanh',
    vnn.r.tanh = 'VNN Tanh',
    vnn.l.tanh = 'VNN Tanh LSUV',
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_0hyps_2trace_data_gmx-phno_StachyoseDry.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_0hyps_2trace_data_gmx-phno_StachyoseDry.png', p, width = 30, height = 5, units = 'in')


p <- plt_net_metrics(md = md, phase_i = 'lightning', phno_i = 'phno_OilDry')
p <- p+scale_y_continuous(
        trans = "log10",
        breaks = c(0.5, 1, 10, 100, 1000))+
    labs(x = '', y = '')+
  facet_grid(phno~init, labeller = labeller(
  phno = c(phno_OilDry = "Oil Dry"),
  init = c(
    dnn.r.tanh = 'DNN Tanh',
    vnn.r.tanh = 'VNN Tanh',
    vnn.l.tanh = 'VNN Tanh LSUV',
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_0hyps_2trace_data_gmx-phno_OilDry.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_0hyps_2trace_data_gmx-phno_OilDry.png', p, width = 30, height = 5, units = 'in')
...
p <- p+scale_y_continuous(
  ...
  init = c(
    ...
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_0hyps_2trace_data_zma-phno_all_Pollen_GDD__FTaxaREnvRGxE.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_0hyps_2trace_data_zma-phno_all_Pollen_GDD__FTaxaREnvRGxE.png', p, width = 30, height = 5, units = 'in')


p <- plt_net_metrics(md = md, phase_i = 'lightning', phno_i = 'phno_all_Yield_Mg_ha')
p <- p+scale_y_continuous(
        trans = "log10",
        breaks = c(0.5, 1, 10, 100, 1000))+
    labs(x = '', y = '')+
  facet_grid(phno~init, labeller = labeller(
  phno = c(phno_all_Yield_Mg_ha = "Yield Mg per ha"),
  init = c(
    dnn.r.tanh = 'DNN Tanh',
    vnn.r.tanh = 'VNN Tanh',
    vnn.l.tanh = 'VNN Tanh LSUV',
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_0hyps_2trace_data_zma-phno_all_Yield_Mg_ha.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_0hyps_2trace_data_zma-phno_all_Yield_Mg_ha.png', p, width = 30, height = 5, units = 'in')


p <- plt_net_metrics(md = md, phase_i = 'lightning', phno_i = 'phno_all_Yield_Mg_ha__FTaxaREnvRGxE')
p <- p+scale_y_continuous(
        trans = "log10",
        breaks = c(0.5, 1, 10, 100, 1000))+
    labs(x = '', y = '')+
  facet_grid(phno~init, labeller = labeller(
  phno = c(phno_all_Yield_Mg_ha__FTaxaREnvRGxE = "Yield Mg per ha Est."),
  init = c(
    dnn.r.tanh = 'DNN Tanh',
    vnn.r.tanh = 'VNN Tanh',
    vnn.l.tanh = 'VNN Tanh LSUV',
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_0hyps_2trace_data_zma-phno_all_Yield_Mg_ha__FTaxaREnvRGxE.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_0hyps_2trace_data_zma-phno_all_Yield_Mg_ha__FTaxaREnvRGxE.png', p, width = 30, height = 5, units = 'in')
...
p <- plt_net_metrics(md = md, phase_i = 'models', phno_i = 'ADHReplicates_hzg', show_min_loss = TRUE)
p <- p+scale_y_continuous(
        trans = "log10",
        breaks = c(0.5, 1, 10, 100, 1000))+
    labs(x = '', y = '')+
  ...
  init = c(
    ...
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_1mods_2trace_data_dme-ADHReplicates_hzg.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_1mods_2trace_data_dme-ADHReplicates_hzg.png', p, width = 30, height = 5, units = 'in')


p <- plt_net_metrics(md = md, phase_i = 'models', phno_i = 'phno_ProteinDry', show_min_loss = TRUE)
p <- p+scale_y_continuous(
        trans = "log10",
        breaks = c(0.5, 1, 10, 100, 1000))+
    labs(x = '', y = '')+
  ...
  init = c(
    ...
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_1mods_2trace_data_gmx-phno_ProteinDry.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_1mods_2trace_data_gmx-phno_ProteinDry.png', p, width = 30, height = 5, units = 'in')


p <- plt_net_metrics(md = md, phase_i = 'models', phno_i = 'phno_StachyoseDry', show_min_loss = TRUE)
p <- p+scale_y_continuous(
        trans = "log10",
        breaks = c(0.5, 1, 10, 100, 1000))+
    labs(x = '', y = '')+
  ...
  init = c(
    ...
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_1mods_2trace_data_gmx-phno_StachyoseDry.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_1mods_2trace_data_gmx-phno_StachyoseDry.png', p, width = 30, height = 5, units = 'in')


p <- plt_net_metrics(md = md, phase_i = 'models', phno_i = 'phno_OilDry', show_min_loss = TRUE)
p <- p+scale_y_continuous(
        trans = "log10",
        breaks = c(0.5, 1, 10, 100, 1000))+
    labs(x = '', y = '')+
  ...
  init = c(
    ...
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_1mods_2trace_data_gmx-phno_OilDry.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_1mods_2trace_data_gmx-phno_OilDry.png', p, width = 30, height = 5, units = 'in')
...
p <- p+scale_y_continuous(
  ...
  init = c(
    ...
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_1mods_2trace_data_zma-phno_all_Pollen_GDD__FTaxaREnvRGxE.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_1mods_2trace_data_zma-phno_all_Pollen_GDD__FTaxaREnvRGxE.png', p, width = 30, height = 5, units = 'in')


p <- plt_net_metrics(md = md, phase_i = 'models', phno_i = 'phno_all_Yield_Mg_ha', show_min_loss = TRUE)
p <- p+scale_y_continuous(
        trans = "log10",
        breaks = c(0.5, 1, 10, 100, 1000))+
    labs(x = '', y = '')+
  ...
  init = c(
    ...
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_1mods_2trace_data_zma-phno_all_Yield_Mg_ha.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_1mods_2trace_data_zma-phno_all_Yield_Mg_ha.png', p, width = 30, height = 5, units = 'in')


p <- plt_net_metrics(md = md, phase_i = 'models', phno_i = 'phno_all_Yield_Mg_ha__FTaxaREnvRGxE', show_min_loss = TRUE)
p <- p+scale_y_continuous(
        trans = "log10",
        breaks = c(0.5, 1, 10, 100, 1000))+
    labs(x = '', y = '')+
  ...
  init = c(
    ...
    vnn.l.relu = 'VNN Relu LSUV',
    vnn.l.none = 'VLN LSUV') ))
ggsave('paper_1mods_2trace_data_zma-phno_all_Yield_Mg_ha__FTaxaREnvRGxE.svg', p, width = 30, height = 5, units = 'in')
ggsave('paper_1mods_2trace_data_zma-phno_all_Yield_Mg_ha__FTaxaREnvRGxE.png', p, width = 30, height = 5, units = 'in')
```
...
# "
# 
# p <- (p_top + p_bot)+plot_layout(design = design)
# 
# poster_1mods_3box <- p
# ggsave('poster_1mods_3box.svg', p, width = 30, height = 5, units = 'in')
...
```{r}
## fast extraction for paper

#FIXME blup for `phno_all_Yield_Mg_ha` is running
# xx <- xx[xx$phn != 'phno_all_Yield_Mg_ha', ]
#FIXME


all_tmp_alls <- map(xx$phn, function(e){
...
all_tmp_all_plts <- map(seq_along(all_tmp_alls), function(i){
      print(i)
  tmp_all <- all_tmp_alls[[i]]
...
#   p <- (p_top + p_bot)+plot_layout(design = design)
  ...
  tmp_best <- left_join(
    (tmp_all |> 
       group_by(data, phno, model, phase, split, init) |>
       mutate(mse = min(median(mse))) |>
       ungroup() |>
       group_by(split) |>
  ...
  # manually figure out the offset for the arrows
  tmp_best <- tmp_best |>
  mutate(init_num = as.numeric(tmp_best$init))  |>
  mutate(init_num = case_when(
    split == 'Train' ~ (init_num - (0.751/4)),
    split != 'Train' ~ (init_num + (0.751/4))
    ))  
  ...
  p <- tmp_all |>
    ggplot(aes(x = init, y = mse, fill = split))+
    geom_hline(
      yintercept = as.numeric( as.list(select(filter(tmp_best, split == 'Train'), mse))[[1]][[1]] ), 
      color = 'cornflowerblue', linetype = 'dashed'
    )+
    geom_hline(
      yintercept = as.numeric( as.list(select(filter(tmp_best, split == 'Validation'), mse))[[1]][[1]] ),
      color = 'orange', linetype = 'dashed'
    )+
    geom_boxplot()+
    geom_segment(
      data = tmp_best, 
      aes(x = init_num, y = 0, xend = init_num, yend = mse, color = split),
      arrow = arrow(length = unit(5, "pt"), type = "closed"),
      # color = "orange",
      linetype = 'solid'
    )+
...
map(seq_along(all_tmp_alls), function(i){
  kegg <- case_when(
    xx$phn[i] %in% c("ADHReplicates_hzg") ~ 'dme',
    xx$phn[i] %in% c("phno_ProteinDry", "phno_StachyoseDry", "phno_OilDry") ~ 'gmx',
    xx$phn[i] %in% c("phno_all_Pollen_GDD__FTaxaREnvRGxE", "phno_all_Yield_Mg_ha__FTaxaREnvRGxE", "phno_all_Yield_Mg_ha") ~ 'zma'
  )
  p <- all_tmp_all_plts[[i]]
  ggsave(paste0('paper_1mods_3box_data_', kegg,'-',xx$phn[i], '.svg'), p, width = 7, height = 7, units = 'in')
  ggsave(paste0('paper_1mods_3box_data_', kegg,'-',xx$phn[i], '.png'), p, width = 7, height = 7, units = 'in')
})
...
pred_parquet_to_df <- function(
    paths = unlist(mdi[, 'path']), 
    infos = unlist(mdi[, 'info']),
    model_type = 'dnn'){
...
# p <- out |>
#   mutate(init = case_when(
#     interaction(model, data) == "lin.data_zma" ~ "BLUP",
#     interaction(model, data) == "vnn.data_zma" ~ "VNN Tanh",
#     interaction(model, data) == "dnn.data_zma" ~ "DNN Tanh",
#     interaction(model, data) == "vnn.data_zma_lsuv_relu" ~ "VNN Relu LSUV",
#     interaction(model, data) == "vnn.data_zma_lsuv_tanh" ~ "VNN Tanh LSUV",
#     interaction(model, data) == "vnn.data_zma_lsuv_vlin" ~ "VLN LSUV"
#   )) |>
#   mutate(init = factor(
#     init, 
#     c("BLUP", "DNN Tanh", "VNN Tanh", "VNN Tanh LSUV", "VNN Relu LSUV", "VLN LSUV") 
...
#     Observed = case_when(
#       model == 'lin' ~   obs,
#       model != 'lin' ~ ((obs_cs * yvar_sd)+yvar_mu),
#     )
#   ) |>
#   ggplot(aes(Observed, yhat, color = split))+
#   geom_point()+
#   geom_smooth(color = 'black', method = 'lm')+
#   geom_abline(slope = 1, intercept = 0, linetype = 'dashed')+
#   facet_grid(split~init)+
#   scale_color_manual(values = c('cornflowerblue', 'orange') )+
#   theme_minimal()+
#   theme(legend.position = 'bottom')
# 
# 
# poster_1mods_4pts <- p
# ggsave('poster_1mods_4pts.svg', p, width = 30, height = 5, units = 'in')
```
...
tmp_fn <- function(
  ...
  # ensure this is only parquet files
  md1 <- md1[str_detect(md1$path, '.parquet$'), ]
  ...
  tmp <- unique(out$data)
  data_kegg =  tmp[str_detect(tmp, 'data_\\D{3}$')]

  p <- out |>
    mutate(init = case_when(
      interaction(model, data) == paste0("lin.", data_kegg) ~ "BLUP",
      interaction(model, data) == paste0("vnn.", data_kegg) ~ "VNN Tanh",
      interaction(model, data) == paste0("dnn.", data_kegg) ~ "DNN Tanh",
      interaction(model, data) == paste0("vnn.", data_kegg, "_lsuv_relu") ~ "VNN Relu LSUV",
      interaction(model, data) == paste0("vnn.", data_kegg, "_lsuv_tanh") ~ "VNN Tanh LSUV",
      interaction(model, data) == paste0("vnn.", data_kegg, "_lsuv_vlin") ~ "VLN LSUV"
    )) |>
    mutate(init = factor(
      init, 
      c("BLUP", "DNN Tanh", "VNN Tanh", "VNN Tanh LSUV", "VNN Relu LSUV", "VLN LSUV") 
...
# "phno_all_Pollen_GDD__FTaxaREnvRGxE"
# "phno_all_Yield_Mg_ha__FTaxaREnvRGxE"
# the ones that fail have the same error:
# 
# Error in `map()`:
# ℹ In index: 1.
...
map(xx$phn, function(e){
  kegg <- case_when(
    e %in% c("ADHReplicates_hzg") ~ 'dme',
    e %in% c("phno_ProteinDry", "phno_StachyoseDry", "phno_OilDry") ~ 'gmx',
    e %in% c("phno_all_Pollen_GDD__FTaxaREnvRGxE", "phno_all_Yield_Mg_ha__FTaxaREnvRGxE", "phno_all_Yield_Mg_ha") ~ 'zma'
  )
  p <- tmp_fn(md = md, phase_i = 'models', phno_i = e)+theme(axis.text.x = element_text(angle = 45, hjust = 1))
  
  ggsave(paste0('paper_1mods_4pts_data_', kegg,'-',e, '.svg'), p, width = 7, height = 7, units = 'in')
  ggsave(paste0('paper_1mods_4pts_data_', kegg,'-',e, '.png'), p, width = 7, height = 7, units = 'in')
  
  p <- p+facet_wrap(split~init, scales = "free", nrow = 2)  
  ggsave(paste0('paper_1mods_4pts_data_', kegg,'-',e, '_scalefree.svg'), p, width = 7, height = 7, units = 'in')
  ggsave(paste0('paper_1mods_4pts_data_', kegg,'-',e, '_scalefree.png'), p, width = 7, height = 7, units = 'in')
...
tmp_fn <- function(
  ...
  out <- out |>
    ...
    group_by(init, model, data, hash, split) |>
    summarise(
      pearson = cor(Observed, yhat, method = 'pearson'),
      spearman = cor(Observed, yhat, method = 'spearman'),
      MSE = mean((Observed - yhat)**2)
    )
  
  return(out)
}

tmp_fn2 <- function(out){
  ##  Pearson ====
  
  tmp_best <- left_join(
    (out |> 
       group_by(data, model, split, init) |>
       mutate(pearson = max(median(pearson))) |>
       ungroup() |>
       group_by(split) |>
       summarize(pearson = max(pearson)) |>
       ungroup() |>
       distinct()
    ),
    (out |> 
  ...
  p1 <- out |>
    ggplot(aes(x = init, y = pearson, fill = split))+
    geom_hline(
      yintercept = as.numeric(select(filter(tmp_best, split == 'Train'), pearson)[[1]][1]), # indexing here ensures that we have a single value in the case of ties
      color = 'cornflowerblue', linetype = 'dashed'
    )+
    geom_hline(
      yintercept = as.numeric(select(filter(tmp_best, split == 'Validation'), pearson)[[1]][1]),
      color = 'orange', linetype = 'dashed'
    )+
    geom_boxplot()+
    geom_segment(
      data = tmp_best, 
      aes(x = init_num, y = 1, xend = init_num, yend = pearson, color = split),
      arrow = arrow(length = unit(5, "pt"), type = "closed"),
      # color = "orange",
      linetype = 'solid'
    )+
  ...
  tmp_best <- left_join(
    (out |> 
       select(-pearson) |>
       group_by(data, model, split, init) |>
       mutate(spearman = max(median(spearman))) |>
       ungroup() |>
       group_by(split) |>
       summarize(spearman = max(spearman)) |>
       ungroup() |>
       distinct()
    ),
    (out |> 
  ...
  p2 <- out |>
    ggplot(aes(x = init, y = spearman, fill = split))+
    geom_hline(
      yintercept = as.numeric(select(filter(tmp_best, split == 'Train'), spearman)[[1]][1]), 
      color = 'cornflowerblue', linetype = 'dashed'
    )+
    geom_hline(
      yintercept = as.numeric(select(filter(tmp_best, split == 'Validation'), spearman)[[1]][1]),
      color = 'orange', linetype = 'dashed'
    )+
    geom_boxplot()+
    geom_segment(
      data = tmp_best, 
      aes(x = init_num, y = 1, xend = init_num, yend = spearman, color = split),
      arrow = arrow(length = unit(5, "pt"), type = "closed"),
      linetype = 'solid'
    )+
    geom_point(position = position_dodge(width=0.751) )+
...
# out = tmp_fn(md = md, phase_i = 'models', phno_i = xx$phn[3])
# (out |> 
#     group_by(init, model, data, split) |>
#        summarise(
...
#   ) |>
#   pivot_wider(
#     id_cols = c('init', 'model', 'data'),
#     names_from = split,
#     values_from = c('pearson', 'spearman', 'MSE')
#   ))
...
map(xx$phn, function(e){
  ...
  print(e)
  out <- tmp_fn(md = md, phase_i = 'models', phno_i = e)
  # summarize
  out_sum1 <- (
    out |>
      group_by(init, model, data, split) |>
  ...
  write.csv(
    out_sum1,
    file = paste0('paper_1mods_7sumlong_', kegg,'-',e, '.csv')
  )  
  
  write.csv(
    out_sum2,
    file = paste0('paper_1mods_8sumwide_', kegg,'-',e, '.csv')
  )  
  ...
  ggsave(paste0('paper_1mods_5cor_data_', kegg,'-',e, '.svg'), p1, width = 7, height = 7, units = 'in')
  ggsave(paste0('paper_1mods_5cor_data_', kegg,'-',e, '.png'), p1, width = 7, height = 7, units = 'in')
  
  ggsave(paste0('paper_1mods_6rho_data_', kegg,'-',e, '.svg'), p2, width = 7, height = 7, units = 'in')
  ggsave(paste0('paper_1mods_6rho_data_', kegg,'-',e, '.png'), p2, width = 7, height = 7, units = 'in')
  
  
  # add text per Jason's recommendation
  p1 <- p1+geom_text(
    data = (out |> 
              group_by(data, model, split, init) |>
              mutate(pearson = min(pearson)) |>
              ungroup() |>
              select(init, split, pearson) |>
              distinct() |>
              mutate(init_num = as.numeric(init))  |>
              mutate(init_num = case_when(
                split == 'Train' ~ (init_num - (0.751/4)),
                split != 'Train' ~ (init_num + (0.751/4))
              ))), 
  ...
  p2 <- p2+geom_text(
    data = (out |> 
              group_by(data, model, split, init) |>
              mutate(spearman = min(spearman)) |>
              ungroup() |>
              select(init, split, spearman) |>
              distinct() |>
              mutate(init_num = as.numeric(init))  |>
              mutate(init_num = case_when(
                split == 'Train' ~ (init_num - (0.751/4)),
  ...
  ggsave(paste0('paper_1mods_5corT_data_', kegg,'-',e, '.svg'), p1, width = 7, height = 7, units = 'in')
  ggsave(paste0('paper_1mods_5corT_data_', kegg,'-',e, '.png'), p1, width = 7, height = 7, units = 'in')
  
  ggsave(paste0('paper_1mods_6rhoT_data_', kegg,'-',e, '.svg'), p2, width = 7, height = 7, units = 'in')
  ggsave(paste0('paper_1mods_6rhoT_data_', kegg,'-',e, '.png'), p2, width = 7, height = 7, units = 'in')
...
```{r}
# all in one heatmap per conversation with Jason

out_hmp <- map(xx$phn, function(e){
  kegg <- case_when(
    e %in% c("ADHReplicates_hzg") ~ 'dme',
    e %in% c("phno_ProteinDry", "phno_StachyoseDry", "phno_OilDry") ~ 'gmx',
    e %in% c("phno_all_Pollen_GDD__FTaxaREnvRGxE", "phno_all_Yield_Mg_ha__FTaxaREnvRGxE", "phno_all_Yield_Mg_ha") ~ 'zma'
...
out_hmp <- do.call(rbind, out_hmp)


out_hmp_mse <- out_hmp |>
  group_by(init, phno, split) |>
  summarise(MSE = median(MSE, na.rm=TRUE)) |>
  filter(!is.na(init), phno != 'phno_all_Yield_Mg_ha') |>
  ungroup() |>
  group_by(phno) |>
  mutate(col_var = log10(MSE / max(MSE))
         ) |>
  ggplot(aes(x = interaction(split, init), y = phno, fill = col_var))+
  geom_tile()+
  geom_text(aes(label = round(MSE, digits = 3)))+
  scale_x_discrete(guide = 'axis_nested')+
  scale_fill_distiller()+
  labs(x = '', y ='', title = 'Median MSE')+
  theme(legend.position = '')


ggsave(paste0('paper_1mods_8htmpmse_data.svg'), out_hmp_mse, width = 11, height = 7, units = 'in')
ggsave(paste0('paper_1mods_8htmpmse_data.png'), out_hmp_mse, width = 11, height = 7, units = 'in')
...
write.csv(
  (out_hmp |>
     ...
     filter(!is.na(init), phno != 'phno_all_Yield_Mg_ha') |>
     ungroup() |>
     pivot_wider(names_from = split, values_from = MSE, names_prefix = 'MSE_') |>
     arrange(phno, init)),
  file = paste0('paper_1mods_8htmpmse_data.csv')
)  


out_hmp_cor <- out_hmp |>
  group_by(init, phno, split) |>
  summarise(pearson = median(pearson, na.rm=TRUE)) |>
  filter(!is.na(init), phno != 'phno_all_Yield_Mg_ha') |>
  ungroup() |>
  group_by(phno) |>
  mutate(col_var = pearson) |>
  ggplot(aes(x = interaction(split, init), y = phno, fill = col_var))+
  geom_tile()+
  geom_text(aes(label = round(pearson, digits = 3)))+
  scale_x_discrete(guide = 'axis_nested')+
  scale_fill_distiller()+
  labs(x = '', y ='', title = 'Median Pearson r')+
  theme(legend.position = '')

ggsave(paste0('paper_1mods_9htmpcor_data.svg'), out_hmp_cor, width = 11, height = 7, units = 'in')
ggsave(paste0('paper_1mods_9htmpcor_data.png'), out_hmp_cor, width = 11, height = 7, units = 'in')
...
write.csv(
  (out_hmp |>
     ...
     filter(!is.na(init), phno != 'phno_all_Yield_Mg_ha') |>
     ungroup() |>
     pivot_wider(names_from = split, values_from = pearson, names_prefix = 'pearson_') |>
     arrange(phno, init)),
  file = paste0('paper_1mods_8htmpcor_data.csv')
)  
...
out_hmp_rho <- out_hmp |>
  ...
  ungroup() |>
  group_by(phno) |>
  mutate(col_var = spearman) |>
  ggplot(aes(x = interaction(split, init), y = phno, fill = col_var))+
  geom_tile()+
  geom_text(aes(label = round(spearman, digits = 3)))+
  scale_x_discrete(guide = 'axis_nested')+
  scale_fill_distiller()+
  labs(x = '', y ='', title = 'Median Spearman ρ')+
  theme(legend.position = '')


ggsave(paste0('paper_1mods_10htmprho_data.svg'), out_hmp_rho, width = 11, height = 7, units = 'in')
ggsave(paste0('paper_1mods_10htmprho_data.png'), out_hmp_rho, width = 11, height = 7, units = 'in')
...
write.csv(
  (out_hmp |>
     ...
     filter(!is.na(init), phno != 'phno_all_Yield_Mg_ha') |>
     ungroup() |>
     pivot_wider(names_from = split, values_from = spearman, names_prefix = 'spearman_') |>
     arrange(phno, init)),
  file = paste0('paper_1mods_8htmprho_data.csv')
)  
...
# phno_all_Pollen_GDD__FTaxaREnv__5_Genotype_Data_All_Years_vals_loci
# phno_all_Pollen_GDD__FTaxaREnv__5_Genotype_Data_All_Years_acgt

f = 'phno_all_Pollen_GDD__FTaxaREnv__5_Genotype_Data_All_Years_acgt'
f = 'phno_all_Pollen_GDD__FTaxaREnv__5_Genotype_Data_All_Years_vals_loci'

gwas <- read.csv(paste0(
  '/home/daniel.kick/daniel/sparsevnn_study/zma_g2f_individual_gapit/output/',
  f,
  '/GAPIT.Association.GWAS_Results.CMLM.Pollen_GDD.csv'
  ))
...
mds <- md |>
  filter(
    phno == 'phno_all_Pollen_GDD__FTaxaREnvRGxE', 
    phase == 'models'
    ) 
...
mds <- full_join(
  (mds |> 
     filter(
    model == 'vnn', 
    data  == 'data_zma_lsuv_tanh',
    info  == 'eval_salience_snpwise_val'
  ) |>
    select(path, hash) |>
    rename(vnn_lsuv_tanh = path)),
  ...
  (mds |> filter(
    ...
    select(path, hash) |>
    rename(dnn_tanh = path))
)


## How correlated is everything?

cor_all <- map(seq(1, nrow(mds)), function(i){
  
  vwas <- read_parquet(as.character(mds[i, 'vnn_lsuv_tanh']))
  dwas <- read_parquet(as.character(mds[i, 'dnn_tanh']))
  
  tmp <- inner_join(
    full_join(
      rename(vwas, vsal = salience),
      rename(dwas, dsal = salience)
      ), 
    (gwas |> 
       as_tibble() |> 
  ...
  cor(tmp$p, tmp$dsal, method = 'pearson')
  cor(tmp$p, tmp$dsal, method = 'spearman')
  tmp <- tmp |>
    select(vsal, dsal, p) 
  ...
  tmp_s <- tmp |>
    ...
    corrr::shave() |>
    pivot_longer(cols = c('vsal', 'dsal', 'p')) |>
    filter(!is.na(value)) |>
    rename(spearman = value) |>
    mutate(hash = as.character(mds[i, 'hash']))
...
cor_all_spearman_sum <- cor_all |> 
  select(-pearson) |>
  mutate(nom = paste(term, name, sep = ' vs ')) |>
  select(nom, spearman, hash) |>
  pivot_wider(names_from = nom, values_from = spearman)


write.csv(
  cor_all_spearman_sum,
  file = paste0('paper_1mods_11gwas_sal_spearman.csv')
)  

cor_all_pearson_sum <- cor_all |> 
  select(-pearson) |>
  mutate(nom = paste(term, name, sep = ' vs ')) |>
  select(nom, spearman, hash) |>
  pivot_wider(names_from = nom, values_from = spearman)
...
write.csv(
  cor_all_pearson_sum,
  file = paste0('paper_1mods_11gwas_sal_pearson.csv')
)  
...
# 
# cor_all |>
#   ggplot(aes(x=name, y=interaction(hash, term), color = spearman))+
#   geom_tile()
# 
# 
# 
...
# ## Visualize
# 
# vwas <- read_parquet(as.character(mds[1, 'vnn_lsuv_tanh']))
# dwas <- read_parquet(as.character(mds[1, 'dnn_tanh']))
# 
# tmp <- inner_join(
#   full_join(
#     rename(vwas, vsal = salience),
...
# 
# p <- tmp |>
#   pivot_longer(cols = c('vsal', 'dsal')) |>
#   ggplot(aes(x = -log10(p), y = value)) +
#   geom_smooth()+
#   # geom_point()+
#   facet_grid(~name)
...
M <- full_join(
  # get layer info from this table
  (select(gradients_nodewise_weights, layer, k) |> distinct() |> rename(node = k)),
  (rho_nodewise_val |> filter(!is.na(Pollen_GDD)))
  ) |>
  filter(!is.na(node_forward_idx))
...
M |> 
  ...
  ggplot(aes(x = Pollen_GDD, y = layer, fill = layer, group = layer))+
...
M |> 
  mutate(layer = as.character(layer)) |>
  ggplot(aes(x = layer, y = Pollen_GDD, color = layer, group = layer))+
...
# Probably want some sort of table
M |> 
  arrange(Pollen_GDD_sig) |>
  group_by(layer) |>
  slice(1:5)
...
M <- (gradients_nodewise_weights |> distinct() |> rename(node = k))
...
M <- full_join(
  # get layer info from this table
  (gradients_nodewise_weights |> distinct() |> rename(node = k)),
  (rho_nodewise_val |> filter(!is.na(Pollen_GDD)))
  ) |>
  filter(!is.na(node_forward_idx))
...
tmp <- map(seq(0, 3), function(i){
  mask <- (M$layer == i)
  
  map(c('pearson', 'spearman'), function(e){
    as.numeric(
      cor(
        M[mask, 'trn_weight_grads'],
        M[mask, 'Pollen_GDD'], 
        method = e
      )) 
  }) |> unlist()
...
map(seq(0, 3), function(i){
  (M |>
     filter(layer == i) |>
     select(trn_weight_grads, Pollen_GDD) |>
     correlate())[1, 2]
})
...
M |>
  select(layer, #node, 
         trn_weight_grads, Pollen_GDD) |>
  mutate(trn_weight_grads = max(trn_weight_grads, na.rm=TRUE)) |>
  group_by(layer) |>
  correlate()
...
M |>
  select(layer, #node, 
         trn_weight_grads, Pollen_GDD) |>
  mutate(trn_weight_grads = mean(trn_weight_grads, na.rm=TRUE)) |>
  group_by(layer) |>
  correlate()
...
```{r}
# subset md to work on just the flowering data initially. 
mds <- md |>
  filter(
    phno == 'phno_all_Pollen_GDD__FTaxaREnvRGxE', 
    phase == 'models'
...
# TODO
# is there an issue with swapping the input data for data_zma_lsuv_tanh?
# it da6512e68f_eval_salience_snpwise is the same across all of the reps.
# maybe it's not getting updated on the eval script?

# TODO run gapit on lambda3
# There's an error with some of the input data, go line by line.




13 eval_salience_snpwise_val      
14 eval_salience_genewise_val     
17 eval_salience_genewise_trn     
18 eval_salience_snpwise_trn      
15 eval_rho_nodewise_val          
16 eval_rho_nodewise_trn          
19 eval_gradients_nodewise_weights
12 eval_gradients_nodewise_bias   


  info                                                 
  <chr>                                                
1 bWGR_yhat                                            
2 GAPIT.Association.GWAS_Results.BLINK.AdjADHSlope.csv 
3 GAPIT.Association.GWAS_Results.BLINK.ProteinDry.csv  
4 GAPIT.Association.GWAS_Results.BLINK.StachyoseDry.csv
5 GAPIT.Association.GWAS_Results.BLINK.OilDry.csv  
...
# a.	What are the top SNPs?
# b.	How well does training SNP salience agree with GWAS? What about with DNN salience?
# c.	Is there anything unexpected when we look at the importance of nodes instead of SNPs?
